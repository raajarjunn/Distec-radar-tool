...
activities_col = db["activities"]   # ← rename to avoid confusion with the earlier import
...
def index(request):
    cursor = (
        activities_col.find(
            {},  # filter if needed
            {"_id": 0, "activity": 1, "username": 1, "occurred_at": 1, "logged_in": 1, "meta": 1}
        )
        .sort("occurred_at", -1)
        .limit(24)  # ← fetch 24 most recent
    )
